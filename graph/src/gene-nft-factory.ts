...
import {
  GeneCreated as GeneCreatedEvent
} from "../generated/GeneNFTFactory/GeneNFTFactory";
import {
  GeneNFT,
  User
...
export function handleGeneCreated(event: GeneCreatedEvent): void {
  ...
  // Use GenesNFT contract address + token ID as the entity ID
  let genesNFTAddress = Address.fromString(GENES_NFT_ADDRESS);
  let geneNFTId = genesNFTAddress.concat(Bytes.fromI32(event.params.geneId.toI32()));
