...
function getUserSessionsFilePath(
  userAddress: string,
  aminalAddress: string
): string {
  return path.join(
    CHAT_DATA_DIR,
    `sessions_${userAddress}_${aminalAddress}.json`
  );
}
...
export async function getUserChatSessions(
  ...
  try {
    const userSessionsPath = getUserSessionsFilePath(
      userAddress,
      aminalAddress
    );
    const data = await fs.readFile(userSessionsPath, 'utf-8');
...
export async function deleteChatSession(sessionId: string): Promise<void> {
  ...
  try {
    ...
    await removeSessionFromUserList(
      session.userAddress,
      session.aminalAddress,
      sessionId
    );
...
export async function deleteAllUserSessions(
  ...
  const deletePromises = sessions.map((session) =>
    fs
      .unlink(getSessionFilePath(session.id))
      .catch((err) =>
  ...
  console.log(
    `🗑️ Deleted all chat sessions for user ${userAddress} with aminal ${aminalAddress}`
  );
}
