...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<
    ChatSession | ChatSession[] | { success: boolean } | { error: string }
  >
) {
  if (req.method === 'POST') {
    ...
    try {
      ...
      if (!aminalAddress || !userAddress) {
        return res
          .status(400)
          .json({ error: 'aminalAddress and userAddress are required' });
      }
      ...
      try {
            ...
            const geneData = Object.entries(geneIds)
              ...
              .map(([geneKey, geneId]) => {
                const gene = genes.find(
                  (g: any) => g.tokenId.toString() === geneId
                );
        ...
        console.error(
          'Error generating gene-based personality for session, using fallback:',
          error
        );
      }
    ...
    try {
      ...
      if (deleteAll === 'true') {
        ...
        if (!sessionId || typeof sessionId !== 'string') {
          return res
            .status(400)
            .json({ error: 'sessionId is required for single delete' });
        }
