...
### 6. MISSING ACCESS CONTROL ON CRITICAL REGISTRY FUNCTIONS

**Severity:** 🟠 HIGH
**Contract:** `Genes.sol:108-111`
**CWE-284:** Improper Access Control

#### Description
The `setRegistry` function allows the contract owner to arbitrarily change the gene registry address at any time without timelock, multi-sig, or governance approval. This creates centralization risk where the owner could:
- Point to a malicious registry
- Block gene creation by setting address(0)
- Redirect gene minting to controlled contracts
- Change registry mid-auction affecting outcomes
...
// Registry used in critical function
function mintGene(...) external returns (uint256 tokenId) {
    // Relies on geneRegistry which owner can change anytime
    require(IGeneRegistry(geneRegistry).isValidGene(...), "Invalid gene");
    // ...
}
```

#### Attack Scenarios
1. Owner changes registry to malicious contract during active auction
2. Malicious registry reports all genes as invalid, blocking minting
3. Owner front-runs high-value gene creation to block competitors
4. Registry changed to contract that steals NFT ownership

#### Impact
- **Centralization:** Owner has unchecked power over gene system
- **Trust:** Users cannot rely on consistent behavior
- **Financial:** Gene creators could lose revenue
- **Manipulation:** Auction outcomes could be influenced

#### Recommended Remediation
```solidity
// Option 1: Two-step transfer with timelock
uint256 public constant REGISTRY_CHANGE_DELAY = 7 days;
address public pendingRegistry;
uint256 public registryChangeTime;

function proposeRegistryChange(address newRegistry) external onlyOwner {
    require(newRegistry != address(0), "Invalid address");
    pendingRegistry = newRegistry;
    registryChangeTime = block.timestamp + REGISTRY_CHANGE_DELAY;
    emit RegistryChangeProposed(newRegistry, registryChangeTime);
}

function executeRegistryChange() external onlyOwner {
    require(block.timestamp >= registryChangeTime, "Timelock active");
    require(pendingRegistry != address(0), "No pending change");

    geneRegistry = pendingRegistry;
    pendingRegistry = address(0);
    emit RegistrySet(geneRegistry);
}

// Option 2: Make immutable after initialization
