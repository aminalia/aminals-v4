    ...
    ReentrancyGuard
{
    ...
    function voteOnGene(
        ...
        if (previousVote > 0) categoryVoting.geneVotes[geneId] -= previousVote;
    ...
    function bulkVoteOnGenes(
        ...
        for (uint256 i = 0; i < 8; i++) {
            if (voteWeights[i] > 0) {
                ...
                for (
                    uint256 j = 0;
                    j < categoryVoting.proposedGenes.length;
                    j++
                ) {
                ...
                // Remove previous vote if exists
                uint256 previousVote = categoryVoting.userVotes[msg.sender][
                    geneId
                ];
                if (previousVote > 0)
    ...
    function _updateCategoryWinner(
        ...
        if (currentVotes > categoryVoting.highestVotes) {
            ...
            currentVotes == categoryVoting.highestVotes && currentVotes > 0
        ) {
    ...
    function settleAuction(
        ...
        uint256 energyFromAminalOne = (aminalOneEnergy *
            ENERGY_TRANSFER_PERCENTAGE) / 100;
        uint256 energyFromAminalTwo = (aminalTwoEnergy *
            ENERGY_TRANSFER_PERCENTAGE) / 100;
        ...
        for (uint256 i = 0; i < 8; i++) {
            ...
            if (
                ...
                if (
                    auction.parentTwoTraits[i] != 0 &&
                    auction.parentTwoTraits[i] != auction.parentOneTraits[i]
                ) {
            ...
            if (selectedGeneId != 0) {
                ...
                if (geneFactory.isValidGene(selectedGeneId)) {
                    uint256 energyPerGene = (energyFromAminalOne +
                        energyFromAminalTwo) / 8;
                    ...
                    if (energyPerGene > 0) {
                        ...
                        try
                            IAminal(aminalOneAddress).transferEnergyToOwner(
                                energyPerGene / 2,
                                geneOwner
                            )
                        {
                        ...
                        try
                            IAminal(aminalTwoAddress).transferEnergyToOwner(
                                energyPerGene / 2,
                                geneOwner
                            )
                        {
